...
export default function FeaturedGame() {
  ...
  useEffect(async () => {
    const response = await axios.get('localhost:3001/api/players/landingpage')
    ...
    <section className="featured-game pt-50 pb-50">
        <div className="container-fluid">
            ...
            <div className="d-flex flex-row flex-lg-wrap overflow-setting justify-content-lg-between gap-lg-3 gap-4"
                data-aos="fade-up">
                  {gameList.map((item) => (
                      <GameItem key={item._id} title={item.name} category={item.category.name} thumbnail={`localhost:3001/uploads/${item.thumbnail}`} />
                      ))}
            </div>
        </div>
    </section>
